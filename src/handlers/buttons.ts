    ...
    if (buttonId === "tickets:create") {
        ...
        await interaction.reply({
            ...
            components: [
                new ActionRowBuilder<ButtonBuilder>().addComponents([
                    new ButtonBuilder()
                        .setLabel("открыть обратно")
                        .setCustomId("tickets:reopen")
                        .setStyle(ButtonStyle.Success),
                    new ButtonBuilder()
                        .setLabel("удалить нахуй")
        ...
        await interaction.editReply(["main:", main].join("\n"));
            ...
            embeds: [
                {
                    ...
                    title: "Поздравляю!",
                    description: ["Вы были приняты на сервер!", "Айпи сервера: `soff.pro`"].join("\n"),
                },
            ],
        });
        ...
        let timeout = setTimeout(
            () =>
                interaction
                    .editReply("Время вышло.")
                    .then(() => setTimeout(() => interaction.deleteReply().catch(() => null), 5_000)),
            30_000,
        );
        const message = (
        ...
        const nick = message.content?.match(/^[a-zA-Z0-9_]{3,16}$/gm)?.[0];

        if (!nick || nick.length !== message.content.length)
            return interaction
                .editReply("Никнейм должен состоять из латинских букв и цифр, не менее 3 и не более 16 символов.")
                .then(() =>
                    setTimeout(() => {
        ...
        if (await User.findOne({ nickname: nick.trim() }))
            return interaction.editReply("Этот никнейм уже используется.").then(() =>
                setTimeout(() => {
                    interaction.deleteReply().catch(() => null);
                    message.delete().catch(() => null);
                }, 5_000),
        ...
        if (!message) return;
        clearTimeout(timeout);
        const age = message.content.match(/^[0-9]{2}$/gm)?.[0];

        if (!age || age.length !== message.content.length)
            return await interaction
                .editReply("Возраст должен состоять из цифр, не менее и не более 2 символов.")
                .then(() =>
                    setTimeout(() => {
                        interaction.deleteReply().catch(() => null);
                        message.delete().catch(() => null);
        ...
        if (parseInt(age) < 13) return await interaction.editReply("дискорд 13+");
        ...
        if (!otkuda || otkuda.length < 8 || otkuda.length > 512)
            return await interaction.editReply("Длина ответа должна составлять от 8 до 512 символов.").then(() =>
                setTimeout(() => {
                    interaction.deleteReply().catch(() => null);
                    message.delete().catch(() => null);
                }, 6_000),
        ...
        if (!skolko || skolko.length < 8 || skolko.length > 512)
            return await interaction.editReply("Длина ответа должна составлять от 8 до 512 символов.").then(() =>
                setTimeout(() => {
                    interaction.deleteReply().catch(() => null);
                    message.delete().catch(() => null);
        ...
        if (!kto || kto.length < 150 || kto.length > 1800)
            return await interaction.editReply("Длина ответа должна составлять от 150 до 1800 символов.").then(() =>
                setTimeout(() => {
                    interaction.deleteReply().catch(() => null);
                    message.delete().catch(() => null);
                }, 6_000),
...
async function check(interaction: ButtonInteraction): Promise<boolean> {
    ...
    const collected = await message.awaitMessageComponent({
        ...
        filter: (b) =>
            (b.user.id === interaction.user.id && b.customId === "tickets:check:yes") ||
            b.customId === "tickets:check:no",
    });
