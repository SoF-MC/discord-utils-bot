...
export const run = async (message: Message<true>, args: string[]) => {
    ...
    try {
        ...
        if (evaluated instanceof Promise) {
            ...
            return evaluated
                .then(async (result: unknown) =>
                    (await botMsg).edit(await generateMessage(message, result, Date.now() - start)),
                )
        ...
        return generateMessage(message, err, null, false)
            .then(reply)
            .then((m) => replies.set(message.id, m));
    }
};
...
async function generateMessage(
    ...
    if (hastebin) {
        ...
        if (res?.status === 200) {
            ...
            return {
                content: `${success ? "✅ Evaluated successfully" : "❌ Javascript failed"}${
                    time ? ` in ${time}ms` : ""
                }: ${url.toString()}`,
                components: [],
            };
    ...
    const collector = message.channel.createMessageComponentCollector({
        filter: (interaction) =>
            interaction.customId === `${identifier}-hastebin` && config.admins.includes(interaction.user.id),
        componentType: ComponentType.Button,
        max: 1,
    });
    collector.on("collect", (interaction) => {
        return void generateMessage(message, result, time, success, true).then((messageOptions) =>
            interaction.update(messageOptions),
        );
    });
    ...
    return {
        ...
        components: [
            new ActionRowBuilder<ButtonBuilder>().setComponents(
                new ButtonBuilder()
                    .setCustomId(`${identifier}-hastebin`)
                    .setStyle(ButtonStyle.Primary)
                    .setLabel("Dump to Hastebin"),
            ),
        ],
    };
}
...
function generateContent(
    ...
    let content: string | null = `${success ? "✅ Evaluated successfully" : "❌ Javascript failed"}${
        time ? ` in ${time}ms` : ""
    }:\`\`\`ansi\n${inspect(result, { colors: true, depth, maxArrayLength })}\`\`\``;

    if (content.length > 2000) {
        if (depth === 1 && Array.isArray(result) && maxArrayLength > 1)
            content = generateContent(result, time, success, depth, maxArrayLength - 1);
