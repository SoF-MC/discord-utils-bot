    ...
    rcon.on("output", async res => {
        ...
        message.channel.send(member.user.toString(), {
            embed: {
                ...
                description: [
                    "Теперь зайдите на сервере по данным ниже, пройдите прямо, нажмите на табличку Билд, позже используйте команду `/p auto` и постарайтесь построить максимально красивую постройку.",
